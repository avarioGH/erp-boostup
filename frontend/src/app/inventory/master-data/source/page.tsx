...
export default function SourcePage() {
  ...
  const [formData, setFormData] = useState<any>({ code: "", name: "", type: "", description: "", isActive: true })
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const res = await MasterDataAPI.getSources()
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await MasterDataAPI.updateSource(editingId, formData)
      } else {
        await MasterDataAPI.createSource(formData)
      }
      setModalOpen(false)
      fetchData()
    } catch (e) {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure?")) {
      try {
        await MasterDataAPI.deleteSource(id)
        fetchData()
      } catch (e) {
        console.error(e)
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-[28px] font-bold tracking-tight text-foreground">Source Master Data</h1>
        </div>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingId ? 'Edit' : 'Add'} Source</DialogTitle>
          </DialogHeader>
