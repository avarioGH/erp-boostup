...
export default function SpeciesPage() {
  ...
  const [formData, setFormData] = useState<any>({ code: "", name: "", description: "", isActive: true })
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const res = await MasterDataAPI.getSpecies()
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await MasterDataAPI.updateSpecies(editingId, formData)
      } else {
        await MasterDataAPI.createSpecies(formData)
      }
      setModalOpen(false)
      fetchData()
    } catch (e) {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure?")) {
      try {
        await MasterDataAPI.deleteSpecies(id)
        fetchData()
      } catch (e) {
        console.error(e)
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-[28px] font-bold tracking-tight text-foreground">Species Master Data</h1>
        </div>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingId ? 'Edit' : 'Add'} Species</DialogTitle>
          </DialogHeader>
