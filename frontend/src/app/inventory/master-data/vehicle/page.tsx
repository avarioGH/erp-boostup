...
export default function VehiclePage() {
  ...
  const [formData, setFormData] = useState<any>({ code: "", plateNumber: "", vehicleType: "", name: "", isActive: true })
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await MasterDataAPI.updateVehicle(editingId, formData)
      } else {
        await MasterDataAPI.createVehicle(formData)
      }
      setModalOpen(false)
      fetchData()
    } catch (e) {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure?")) {
      try {
        await MasterDataAPI.deleteVehicle(id)
        fetchData()
      } catch (e) {
        console.error(e)
      }
  ...
  const openNew = () => {
    setFormData({ code: "", plateNumber: "", vehicleType: "", name: "", isActive: true })
    setEditingId(null)
    setModalOpen(true)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-[28px] font-bold tracking-tight text-foreground">Vehicle Master Data</h1>
        </div>
