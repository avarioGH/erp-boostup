...
import { ArrowLeft, Save, Scale, AlertCircle, CheckCircle2, Plus, Trash2 } from "lucide-react"
...
export default function StockAdjustmentPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    notes: "",
    items: [
      { productId: "", qty: 1, unitCost: 0, notes: "", adjustmentType: "OUT" }
    ]
  })
  ...
  const addItem = () => {
    setFormData(prev => ({
      ...prev,
      items: [...prev.items, { productId: "", qty: 1, unitCost: 0, notes: "", adjustmentType: "OUT" }]
    }))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const item of formData.items) {
      if (item.adjustmentType === 'OUT') {
        ...
        if (item.qty > available) {
          ...
          setError(`Stok tidak mencukupi untuk penyesuaian defisit (OUT) produk ${prod?.name}. Tersedia: ${available}`)
          setLoading(false)
          return
        }
      }
    ...
    try {
      await api.post('/inventory/adjustment', formData)
      setSuccess(true)
      setTimeout(() => {
      ...
      console.error(err)
      setError(err.response?.data?.message || "Gagal menyimpan transaksi penyesuaian.")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 max-w-4xl mx-auto pb-10">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Penyesuaian Stok</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Lakukan Stock Opname untuk menyesuaikan selisih stok fisik vs sistem.</p>
        </div>
      </div>

      <Card className="border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden group hover:border-purple-500 transition-colors">
        <div className="h-2 w-full bg-purple-500"></div>
        <form onSubmit={handleSubmit}>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Scale className="w-5 h-5 text-purple-500" /> Detail Penyesuaian
            </CardTitle>
            <CardDescription>Pilih gudang, jenis penyesuaian (Surplus/Defisit), dan rincian barang.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            {success && (
              <div className="bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 p-4 rounded-lg flex items-center gap-3 border border-emerald-200 dark:border-emerald-800">
                ...
                <p className="font-medium text-sm">Penyesuaian stok berhasil disimpan! Mengalihkan...</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <Input 
                  placeholder="Misal: Hasil audit Q3" 
            ...
            <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800">
              <div className="flex items-center justify-between">
                <Label className="text-base font-semibold">Daftar Barang yang Disesuaikan</Label>
                  ...
                  <div key={index} className="flex flex-col sm:flex-row gap-3 items-start sm:items-end bg-slate-50 dark:bg-slate-900/50 p-4 rounded-lg border border-slate-200 dark:border-slate-800 relative">
                            ...
                            return (
                              <SelectItem key={p.id} value={p.id}>
                                <div className="flex justify-between items-center w-full">
                                  <span>{p.name}</span>
                                  <span className="text-xs text-slate-400 ml-2">(Sistem: {avail})</span>
                                </div>
                              </SelectItem>
                            )
                          })}
                    ...
                    <div className="space-y-2 w-full sm:w-32">
                      ...
                      <Select value={item.adjustmentType} onValueChange={(val) => updateItem(index, 'adjustmentType', val as string)}>
                        ...
                        <SelectContent>
                          <SelectItem value="IN" className="text-emerald-600 font-medium">Surplus (+)</SelectItem>
                          <SelectItem value="OUT" className="text-rose-600 font-medium">Defisit (-)</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>

                    <div className="space-y-2 w-full sm:w-24">
                      <Label className="text-xs">Kuantitas Selisih</Label>
                      <Input 
                        type="number" min="1" 
                        max={item.adjustmentType === 'OUT' ? getAvailableStock(item.productId) : undefined}
                        value={item.qty}
                        onChange={(e) => updateItem(index, 'qty', Number(e.target.value))}
                        className={`bg-white dark:bg-slate-950 border-slate-300 dark:border-slate-800 ${(item.adjustmentType === 'OUT' && item.qty > getAvailableStock(item.productId)) ? 'border-rose-500 text-rose-500' : ''}`}
                      />
                    </div>
            ...
            <div className="flex justify-end gap-3 pt-6 border-t border-slate-200 dark:border-slate-800">
              ...
              <Button type="submit" disabled={loading} className="bg-purple-600 hover:bg-purple-700 text-white min-w-[140px]">
                {loading ? "Memproses..." : (
                  <span className="flex items-center gap-2"><Save className="w-4 h-4" /> Simpan Penyesuaian</span>
                )}
              </Button>
            </div>
          </CardContent>
