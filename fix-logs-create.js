const fs = require('fs');
let code = fs.readFileSync('frontend/src/app/inventory/logs/create/page.tsx', 'utf-8');
...
// Title styling
code = code.replace(
  /<h1 className="text-\[28px\] font-bold tracking-tight text-foreground">Register Raw Logs<\/h1>/,
  '<h1 className="text-xl md:text-2xl font-bold tracking-tight text-foreground">Register Raw Logs</h1>'
);
code = code.replace(
  /<p className="text-muted-foreground mt-1">Mass entry form for receiving raw timber logs\.<\/p>/,
  '<p className="text-sm text-muted-foreground">Penerimaan kayu bulat / DUKB mass entry.</p>'
);
...
fs.writeFileSync('frontend/src/app/inventory/logs/create/page.tsx', code);
console.log('Log Kayu (Create) updated');
