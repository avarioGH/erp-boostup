...
# Remove old injected tests
c = re.sub(r'  // ==========================================\n  // STEP 20F.2F PURCHASING.*?console\.log\(\'\\nTEST CASE COUNT\'\);', '  console.log(\'\\nTEST CASE COUNT\');', c, flags=re.DOTALL)
...
test_code = """
  ...
  await prisma.warehouse.create({ data: { id: w1, company_id: c1, name: 'Main WH', type: 'INTERNAL' } });
  await prisma.supplier.create({ data: { id: s1, company_id: c1, name: 'Test Supplier', code: 'S-01' } });
  await prisma.product.create({ data: { id: p1, company_id: c1, name: 'Test Product', sku: 'P-01', type: 'GOODS', buy_price: 5000000, category_id: (await prisma.category.create({data:{company_id:c1, name:'Cat'}})).id } });
  
  const invAcc = await gl.getAccount(c1, '1-1300', '1300', 'Inventory', 'Asset');
  const apAcc = await gl.getAccount(c1, '2-1100', '2100', 'AP', 'Liability');
  const expAcc = await gl.getAccount(c1, '6-2001', '6001', 'Expense', 'Expense');
  const cashAcc = await gl.getAccount(c1, '1-1100', '1100', 'Cash', 'Asset');
  ...
  await verify('PURCHASING', 'P1-P8 - Purchase Request lifecycle', async () => {
    const pr = await purchasingService.createPurchaseRequest(c1, {
      requiredDate: new Date(), source: 'MANUAL', reason: 'Need laptops', warehouseId: w1,
  ...
  await verify('PURCHASING', 'P23-P34 - Goods Receipt & Inventory', async () => {
    const gr = await purchasingService.receiveGoods(c1, poId, {
      warehouseId: w1, items: [{ productId: p1, qty: 10 }]
    });
  ...
  await verify('PURCHASING', 'P35-P43 - Vendor Bill & AP', async () => {
    const bill = await purchasingService.createVendorBill(c1, poId, {
      dueDate: new Date(), items: [{ productId: p1, qty: 10 }]
    });
