...
async function run() {
  ...
  // Setup data
  const company = await prisma.company.create({ data: { name: 'Test Co', code: 'TC', currency: 'IDR' } });
  const companyId = company.id;

  const role = await prisma.role.create({ data: { company_id: companyId, name: 'Admin', is_system: true, permissions: [] } });
  const user = await prisma.user.create({ data: { company_id: companyId, role_id: role.id, name: 'Admin', email: 'admin@test.com', password: 'hash', status: true } });
  ...
  const cat = await prisma.category.create({ data: { company_id: companyId, code: 'CAT', name: 'Cat', status: true, type: 'PRODUCT' } });
  const pa = await prisma.product.create({ data: { company_id: companyId, code: 'PRODA', name: 'Product A', category_id: cat.id, type: 'STOCKED', status: true, can_sell: true, can_purchase: true, base_price: 100 } });
  ...
  try {
    ...
    verifyCondition('T1', 'NORMAL GOODS RECEIPT', s?.current_stock === 10 && s?.available_stock === 10, 'Stock is 10');
    ...
    verifyCondition('T2', 'NORMAL DELIVERY', s?.current_stock === 7 && s?.available_stock === 7, 'Stock is 7');
    ...
    try {
      ...
      s = await prisma.warehouseStock.findFirst({ where: { warehouse_id: warehouseA, product_id: productA } });
      verifyCondition('T3', 'NEGATIVE STOCK PREVENTED', s?.current_stock === 5, 'Transaction rolled back successfully');
    }
    ...
    s = await prisma.warehouseStock.findFirst({ where: { warehouse_id: warehouseA, product_id: productA } });
    verifyCondition('T4', 'RESERVATION', s?.current_stock === 10 && s?.reserved_stock === 7 && s?.available_stock === 3, 'Reservation updated correctly');

    // T5. TRANSFER
    await reset();
    await prisma.\\(async (tx) => {
      await inventoryService.receiveStock(tx as any, { companyId, warehouseId: warehouseA, productId: productA, quantity: 10, unitCost: 100, referenceType: 'GRN', referenceId: 'grn-1', userId });
      await inventoryService.transferStock(tx as any, { companyId, sourceWarehouseId: warehouseA, destinationWarehouseId: warehouseB, productId: productA, quantity: 4, referenceType: 'TRANSFER', referenceId: 'tr-1', userId });
    });
    ...
    verifyCondition('T5', 'TRANSFER', s1?.current_stock === 6 && s2?.current_stock === 4, 'Stock transferred successfully');
