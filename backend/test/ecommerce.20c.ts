...
import { TripayModule } from '../src/integrations/providers/payment/tripay/tripay.module';

const pass = (id: string, msg: string) => console.log(`  ${id.padEnd(4)} | [PASS] | ${msg}`);
const fail = (id: string, msg: string) => { console.error(`  ${id.padEnd(4)} | [FAIL] | ${msg}`); process.exit(1); };
const skip = (id: string, msg: string) => console.log(`  ${id.padEnd(4)} | [SKIP] | ${msg}`);
...
async function run() {
  ...
  const moduleFixture: TestingModule = await Test.createTestingModule({ imports: [AppModule, EcommerceModule, TripayModule] }).compile();
  ...
  const wh = await prisma.warehouse.create({ data: { company_id: compA.id, name: 'WH', code: 'WH' } });
  const unit = await prisma.unit.create({ data: { company_id: compA.id, name: 'Pcs' } });
  
  // Tripay Provider
  await prisma.integration.create({
    data: { company_id: compA.id, provider: 'TRIPAY', status: 'ACTIVE', config: { api_key: 'fake', private_key: 'fake', merchant_code: 'fake' } }
  });

  const setupProduct = async (name: string, qty: number, price: number, company = compA) => {
    const p = await prisma.product.create({ data: { company_id: company.id, name, code: `P-${Date.now()}-${Math.random()}`, unit_id: unit.id, selling_price: price, purchase_price: 0, status: true, is_published: true } });
    const w = await prisma.warehouseStock.create({ data: { company_id: company.id, product_id: p.id, warehouse_id: wh.id, current_stock: qty, available_stock: qty, reserved_stock: 0 } });
    return { p, w };
  };
  ...
  const verifyStock = async (stockId: string, expectedCurrent: number, expectedReserved: number, expectedAvailable: number, id: string) => {
    ...
    if (s.current_stock === expectedCurrent && s.reserved_stock === expectedReserved && s.available_stock === expectedAvailable) {
      pass(id, `Stock verified: cur=${s.current_stock} res=${s.reserved_stock} avail=${s.available_stock}`);
    } else {
      fail(id, `Stock mismatch: got cur=${s.current_stock} res=${s.reserved_stock} avail=${s.available_stock}, expected ${expectedCurrent},${expectedReserved},${expectedAvailable}`);
    }
  ...
  Object.getPrototypeOf(tripayService).createPaymentRequest = async (...args) => {
    tripayCallCount++;
  ...
  await cartService.addItem(compA.id, 'sessionA', pA.id, 2);
  ...
  await cartService.addItem(compA.id, 'sessionB', pB.id, 10);
  try { await checkoutService.checkout(compA.id, 'sessionB', { email: 'b@b.com', name: 'Test', fail_at: 'NONE' }); fail('B', 'Should fail'); } 
  ...
  await cartService.addItem(compA.id, 'sessionC', pC.id, 2);
  try { await checkoutService.checkout(compA.id, 'sessionC', { email: 'c@c.com', name: 'Test', fail_at: 'INVOICE' }); fail('C', 'Should fail'); } 
  ...
  await cartService.addItem(compA.id, 'sessionD', pD.id, 2);
  try { await checkoutService.checkout(compA.id, 'sessionD', { email: 'd@d.com', name: 'Test', fail_at: 'SALES_ORDER' }); fail('D', 'Should fail'); } 
  ...
  await cartService.addItem(compA.id, 'sessionE', pE.id, 2);
  try { await checkoutService.checkout(compA.id, 'sessionE', { email: 'e@e.com', name: 'Test', fail_at: 'CUSTOMER' }); fail('E', 'Should fail'); } 
  ...
  await cartService.addItem(compA.id, 'sessionF', pF.id, 2);
  const resF1 = await checkoutService.checkout(compA.id, 'sessionF', { email: 'f@f.com', name: 'Test', fail_at: 'NONE' });
  const resF2 = await checkoutService.checkout(compA.id, 'sessionF', { email: 'f@f.com', name: 'Test', fail_at: 'NONE' });
  ...
  await cartService.addItem(compA.id, 'sessionG', pG.id, 2);
  const p1 = checkoutService.checkout(compA.id, 'sessionG', { email: 'g@g.com', name: 'Test', fail_at: 'NONE' });
  const p2 = checkoutService.checkout(compA.id, 'sessionG', { email: 'g@g.com', name: 'Test', fail_at: 'NONE' });
  ...
  await cartService.addItem(compA.id, 'sessionH1', pH.id, 7);
  await cartService.addItem(compA.id, 'sessionH2', pH.id, 7);
  const [resH1, resH2] = await Promise.all([
    checkoutService.checkout(compA.id, 'sessionH1', { email: 'h1@h.com', name: 'Test', fail_at: 'NONE' }).catch(e=>e),
    checkoutService.checkout(compA.id, 'sessionH2', { email: 'h2@h.com', name: 'Test', fail_at: 'NONE' }).catch(e=>e)
  ]);
  const successes = [resH1, resH2].filter(r => r.success).length;
  if (successes === 1) pass('H', 'Concurrent OCC protected oversell'); else fail('H', `Expected 1 success`);
  await verifyStock(wH.id, 10, 7, 3, 'H');

  // I. Cross-tenant key lookup
  const resI = await checkoutService.checkout(compB.id, 'sessionF', { email: 'f@f.com', name: 'Test', fail_at: 'NONE' }).catch(e=>e);
  ...
  await cartService.addItem(compA.id, 'sessionL', pL.id, 2);
  const resL = await checkoutService.checkout(compA.id, 'sessionL', { email: 'l@l.com', name: 'Test', fail_at: 'NONE' });
  ...
  // M. Tripay retry
  const resM = await checkoutService.checkout(compA.id, 'sessionL', { email: 'l@l.com', name: 'Test', fail_at: 'NONE' });
