...
async function run() {
  ...
  await prisma.product.create({ data: { id: p_1, company_id: c1, name: 'Test Product', code: 'P01', selling_price: 10000, purchase_price: 5000000, unit_id: new ObjectId().toHexString(), category_id: (await prisma.category.create({data:{company_id:c1, name:'Cat'} as any})).id } as any });
    ...
    await verify('ASSET', 'A1-A8 - Asset Capitalization', async () => {
      ...
      const asset = await prisma.assetMaster.create({
        data: {
          ...
          purchase_price: 20000000, condition: 'EXCELLENT', is_capitalized: false, status: 'ACTIVE'
        } as any
      });
  ...
  if (approvalService) {
    await verify('APPROVAL', 'AP1-AP14 - Approval Engine', async () => {
      const dummyPo = await prisma.purchaseOrder.create({ data: { company_id: c1, order_number: 'PO-DUMMY', status: 'DRAFT', supplier_id: s_1, order_date: new Date(), total_amount: 0 } as any });
      ...
      await prisma.user.create({ data: { id: uApproverId, username: 'appr', name: 'Appr', email: 'a@b.com', password: 'x',  } as any });
