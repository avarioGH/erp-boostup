...
async function run() {
  ...
  await prisma.product.createMany({ data: [
    { id: p1, company_id: c1, name: 'Prod1', purchase_price: 50000 },
    { id: p1.replace('1','2'), company_id: c2, name: 'Prod1_B', purchase_price: 50000 }
  ]});
  
  await prisma.warehouse.createMany({ data: [
    { id: w1, company_id: c1, name: 'Main A', code: 'W1' },
    { id: w2, company_id: c1, name: 'Sub A', code: 'W2' },
    { id: w1.replace('1','2'), company_id: c2, name: 'Main B', code: 'W3' }
  ]});
  ...
  const seedLayers = async (comp = c1, prod = p1, wh = w1) => {
    ...
    await prisma.inventoryCostLayer.create({ data: { company_id: comp, warehouse_id: wh, product_id: prod, quantity: 100, remaining_quantity: 100, unit_cost: 10000, layer_date: new Date('2024-01-01') }});
    await prisma.inventoryCostLayer.create({ data: { company_id: comp, warehouse_id: wh, product_id: prod, quantity: 100, remaining_quantity: 100, unit_cost: 12000, layer_date: new Date('2024-01-02') }});
  };
  ...
  try {
    ...
    const res: any = await prisma.$transaction(tx => consumeFifoLayers(tx as any, { companyId: c1, productId: p1, warehouseId: w1, quantity: 100, stockMovementId: 'sm_A' }));
  ...
  try {
    await resetDB(); await seedLayers();
    const res: any = await prisma.$transaction(tx => consumeFifoLayers(tx as any, { companyId: c1, productId: p1, warehouseId: w1, quantity: 150, stockMovementId: 'sm_B' }));
  ...
  try {
    await resetDB(); await seedLayers();
    await prisma.$transaction(tx => consumeFifoLayers(tx as any, { companyId: c1, productId: p1, warehouseId: w1, quantity: 200, stockMovementId: 'sm_C' }));
  ...
  try {
    ...
    let threw = false;
    try {
      await prisma.$transaction(tx => consumeFifoLayers(tx as any, { companyId: c1, productId: p1, warehouseId: w1, quantity: 300, stockMovementId: 'sm_D' }));
  ...
  try {
    await resetDB(); await seedLayers();
    await prisma.$transaction(tx => consumeFifoLayers(tx as any, { companyId: c1, productId: p1, warehouseId: w1, quantity: 50, stockMovementId: 'sm_E' }));
  ...
  try {
    await resetDB(); await seedLayers();
    await prisma.$transaction(tx => transferFifoLayers(tx as any, { companyId: c1, productId: p1, sourceWarehouseId: w1, destWarehouseId: w2, quantity: 150, sourceMovementId: 's1', destMovementId: 's2' }));
  ...
  try {
    await resetDB(); await seedLayers();
    const res: any = await prisma.$transaction(tx => consumeFifoLayers(tx as any, { companyId: c1, productId: p1, warehouseId: w1, quantity: 150, stockMovementId: 'sm_U' }));
  ...
  } catch(e: any) { verifyFail('G', 'InventoryService.createTransfer', e.message); }
  ...
  try {
    ...
    await prisma.inventoryCostLayer.create({ data: { company_id: c1, warehouse_id: w1, product_id: p1, quantity: 100, remaining_quantity: 100, unit_cost: 10000, layer_date: new Date('2024-01-01') }});
    ...
    await prisma.inventoryCostLayer.create({ data: { company_id: c1, warehouse_id: w2, product_id: p1, quantity: 100, remaining_quantity: 100, unit_cost: 20000, layer_date: new Date('2024-01-01') }});
  ...
  try {
    ...
    let threw = false;
    try {
      await prisma.$transaction(tx => consumeFifoLayers(tx as any, { companyId: c1, productId: p1, warehouseId: w1, quantity: -50, stockMovementId: 's_inv' }));
    } catch (e: any) { threw = true; }
  ...
  try {
    ...
    if (layer?.remaining_quantity === 50 && Number(layer?.unit_cost) === 12000 && layer?.remaining_quantity * Number(layer?.unit_cost) === 600000) {
  ...
  try {
    ...
    const hasIdx = (ix as any).cursor.firstBatch.find((i: any) => i.name.includes("idempotency_key"));
