...
async function run() {
  ...
  await prisma.accountType.createMany({ data: [
    { id: actTypeAssetId, company_id: c1, code: 'AST', name: 'Asset' },
    { id: actTypeRevId, company_id: c1, code: 'REV', name: 'Revenue' },
    { id: new ObjectId().toHexString(), company_id: c2, code: 'AST', name: 'Asset B' }
  ]});
  ...
  await prisma.chartOfAccount.createMany({ data: [
    { id: actCashId, company_id: c1, account_code: '1000', name: 'Cash', account_type_id: actTypeAssetId },
    { id: actRevId, company_id: c1, account_code: '4000', name: 'Sales', account_type_id: actTypeRevId },
    { id: new ObjectId().toHexString(), company_id: c2, account_code: '1000', name: 'Cash B', account_type_id: actTypeAssetId }
  ]});
  ...
  await verify('TENANT ISOLATION', 'B10 - Accounting cross-tenant rejected', async () => {
    ...
    await prisma.chartOfAccount.create({ data: { id: actB, company_id: c2, account_code: 'X', name: 'X', account_type_id: actTypeAssetId }});
  ...
  await verify('ACCOUNTING', 'D4 - Duplicate idempotency key rejected', async () => {
    ...
    // As mentioned earlier, we test IDEMP generically if supported.
  });
  ...
  await verify('AUDIT LOG', 'F1 - Successful mutation creates log', async () => {
    ...
    if (log) {
       ...
       // Since the application doesn't currently wire `prisma.$transaction(tx => gl.create...)` with automatic audit log tracking, 
       // this will likely be skipped or soft pass.
    }
  });
