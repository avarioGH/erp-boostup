...
injection = """
  ...
  const uAdminId = new ObjectId().toHexString(); // In case we need one

  await prisma.user.create({ data: { id: uAdminId, company_id: c1, username: 'tester_'+Date.now(), name: 'Tester', email: 'test_'+Date.now()+'@internal', password: 'pwd', role_id: new ObjectId().toHexString() }});
  ...
  await verify('TRANSFER', 'F1-F10 - Stock transfer mechanics', async () => {
    ...
    const movOut = await prisma.stockMovement.findFirst({ where: { product_id: prod2, warehouse_id: wh1, movement_type: 'OUT' }, orderBy: { created_at: 'desc' } });
    const movIn = await prisma.stockMovement.findFirst({ where: { product_id: prod2, warehouse_id: wh2, movement_type: 'IN' }, orderBy: { created_at: 'desc' } });
